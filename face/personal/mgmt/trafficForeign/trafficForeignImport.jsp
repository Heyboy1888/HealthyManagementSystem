...
<ls:body>
	<ls:form name="templetAddForm" id="templetAddForm" enctype="multipart/form-data">
		<ls:text name="codeColour" property="codeColour"  type="hidden"/>
	...
	<ls:script>
		...
		         return;    
		     } 
			var codeColour=templetAddForm.fields.codeColour.value;
			templetAddForm.submit('~/temTrafficFromForeignController/redCodeUpload?codeColour='+codeColour,function(data){
				...
				if(flag){//成功的时候才取值,否则会出现JS异常
					...
					//LS.message("info","文件导入成功,导入总数量:"+_allCount+"个，增加"+_scuuessCount+"个");
		...
		function changeTxt(a,b,c){
		  ...
		  _interval = setInterval(function(){
		  				var codeTag = $('#jGrowl > div.jGrowl-notification.ui-state-confirm.ui-corner-all.confirm > div.jGrowl-message > div.jGrowl-message-cont',parent.parent.document);
	   	  				...
	   	  				if(codeTag.html()){
	   	  					codeTag.html('<p style="line-height:15px; vertical-align:middle;">文件导入成功!<br>本次导入总数量 : <span style="color:#F00;font-weight:700;font-size:18px;">'+a+'</span>个；</br>成功：<span style="color:#F00;font-weight:700;font-size:18px;">'+b+'</span>个');
	   	  					codeCancel.remove();
